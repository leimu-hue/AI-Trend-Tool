...
- **THEN** 表格每行显示：名称（`color: var(--fg)`）、最后使用（mono 字体）、过期时间（mono 字体，「永久」或日期）、状态（`.badge-success` 有效 / `.badge-danger` 已吊销）、操作按钮（有效 Token 显示「复制」「吊销」，已吊销显示「—」）
...
- **THEN** 弹出 `window.confirm('确定要吊销令牌 "{name}" 吗？')`，确认后调用 `POST /api/v1/tokens/revoke/{id}`，刷新列表，显示"令牌已吊销"Toast
