...
The system SHALL run the Pusher module as an asynchronous background task on a configurable interval (default 10 seconds) and expose a shared function for manual triggering.

#### Scenario: Pusher runs on interval

- **WHEN** the Pusher loop is running
- **THEN** it SHALL call `run_pusher_once` every `config.pusher.interval_seconds`
