...
Defines Rust data model structs for all 8 database tables under `src/models/`. Each struct derives `sqlx::FromRow` for database query mapping and `serde::Serialize` for API responses. Request DTOs (`Create*Request`, `Update*Request`) separate user input from database entities. The module follows the 2018 edition Rust convention (`src/models.rs` + `src/models/`).
