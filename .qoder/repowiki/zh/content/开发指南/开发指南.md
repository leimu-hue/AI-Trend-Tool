...
## 简介
本指南面向参与“AI趋势监控系统”开发的工程师，系统性阐述代码结构、模块划分、开发约定与最佳实践。重点覆盖：
...
## 项目结构
项目采用“分层+模块化”的组织方式：
- 入口与配置：main.rs、config.rs、config.toml
...
## 架构总览
系统采用“采集-过滤-推送”的流水线架构，三类后台任务可独立或组合运行；API服务提供认证与查询能力。
...
- CORS：使用tower-http的CORS中间件设置为宽松策略。
...
## 结论
本项目以清晰的分层与模块化设计实现了RSS采集、关键词匹配与Webhook推送的完整流水线。通过Tokio运行时与信号量等并发原语，结合统一错误处理与响应格式，保证了系统的稳定性与可维护性。遵循本文的开发流程与最佳实践，可高效地扩展新功能模块并持续演进。
